...
const ListingDetailPage = () => {
  ...
  const fetchPropertyDetails = async () => {
    ...
    try {
      const response = await fetch(`https://estate-70yd.onrender.comapi/property/${propertyId}`); // Replace with your endpoint
  ...
  return (
    ...
    {property && property.imageUrls && property.imageUrls.map((imageUrl, index) => (
      ...
      >
        <img src={`https://estate-70yd.onrender.com${imageUrl}`} alt={`Slide ${index + 1}`} className="d-block w-100" />
      </button>
    ))}
  </div>
  <div className="carousel-inner listing-inner">
    {property && property.imageUrls && property.imageUrls.map((imageUrl, index) => (
      <div key={index} className={`carousel-item ${index === 0 ? "active" : ""}`}>
        <img src={`https://estate-70yd.onrender.com${imageUrl}`} className="d-block w-100 h-100" alt={`Image ${index + 1}`} />
      </div>
    ))}
  </div>  
        <button
